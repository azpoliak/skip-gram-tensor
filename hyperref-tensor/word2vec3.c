...
//
// Modifed by Ryan Cotterel & Adam Poliak, Sept-Dec 2016
// Removed:
//    hierarchical-softmax training
//    cbow
// Added:
//   - allowed for more contexts and tensor factorization
//
/////////////////////////////////////////////////////////////////
//
...
char dumpcv_file[MAX_STRING], dumprv_file[MAX_STRING];
...
void InitNet(struct vocabulary *wv, struct vocabulary *cv, struct vocabulary *rv) {
   long long a, b;
   ...
   a = posix_memalign((void **)&syn2, 128, (long long)rv->vocab_size * layer1_size * sizeof(real));
   if (syn0 == NULL) {printf("Memory allocation failed\n"); exit(1);}
   for (b = 0; b < layer1_size; b++)
      for (a = 0; a < rv->vocab_size; a++)
        syn2[a * layer1_size + b] = 1.0 ; //(rand() / (real)RAND_MAX - 0.5) / layer1_size;
...
void *TrainModelThread(void *id) {
  int ctxi = -1, wrdi = -1, rlti = -1;
  long long d;
  long long word_count = 0, last_word_count = 0;
  long long l1, l2, l3, c, target, label;
  ...
  for (iter=0; iter < numiters; ++iter) {
	...
	rlti = ReadWordIndex(rv, fi);

        word_count++; //TODO ?
        if (wrdi < 0 || ctxi < 0 || rlti < 0) {
	  continue;
	}
	...
	/*    } */
        ...
        // NEGATIVE SAMPLING
        l1 = wrdi * layer1_size;
	// TODO: Ryan added this bit below: is it right?
	l3 = rlti * layer1_size;
        for (d = 0; d < negative + 1; d++) {
           if (d == 0) {
              ...
              if (target == 0) {
		target = next_random % (cv->vocab_size - 1) + 1;
	      }
              if (target == ctxi) {
		continue;
	      }
	   ...
	   // gets the function value
           ...
           for (c = 0; c < layer1_size; c++) {
	     if (tensor == 1) {
	       f += syn0[c + l1] * syn1neg[c + l2] * syn2[c + l3];
	     } else {
	   ...
	   if (f > MAX_EXP) {
	     ...
	     g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
           ...
           for (c = 0; c < layer1_size; c++) {
	     if (tensor == 1) {
	       neu1e[c] += g * syn1neg[c + l2] * syn2[c + l3];
	       neu2[c] += g *  syn1neg[c + l2] * syn0[c + l1];
	       syn1neg[c + l2] += g * syn0[c + l1] * syn2[c + l3];
	     } else {
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\nExamples:\n");
    printf("./word2vecf -train data.txt -wvocab wv -cvocab cv -rvocab rv -output vec.txt -size 200 -negative 5 -threads 10 \n\n");
    return 0;
  }
  output_file[0] = 0;
  wvocab_file[0] = 0;
  ...
  if ((i = ArgPos((char *)"-dumprv", argc, argv)) > 0) strcpy(dumprv_file, argv[i + 1]);
