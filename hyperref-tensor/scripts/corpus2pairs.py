        ...
        for line in f:
            ...
            for i, tok in enumerate(tokens):
                if tok is not None:
                    ...
                    if pos:
                        output = []
                        output += [row for row in [tok + ' ' + tokens[j] + ' ' + str(j - i) for j in xrange(start, end) if j != i and tokens[j] is not None] if len(row) > 0]
                    else:
                        output = []
                        output += [row for row in [tok + ' ' + tokens[j] + ' LEFT' for j in xrange(start, i) if j != i and tokens[j] is not None] if len(row) > 0]
                        output += [row for row in [tok + ' ' + tokens[j] + ' RIGHT' for j in xrange(i, end) if j != i and tokens[j] is not None] if len(row) > 0]

                        
                        
                    if len(output) > 0:
                        print '\n'.join(map(lambda x: x.strip(), output))
